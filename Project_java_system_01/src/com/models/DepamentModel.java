...
public class DepamentModel {
	// findAll
	public List<Department> findAll(){
		List<Department> depaments = new ArrayList<Department>();
		try {
			PreparedStatement preparedStatement = ConnectDB.connection().prepareStatement("select * from department");
			ResultSet resultSet = preparedStatement.executeQuery();
			while(resultSet.next()) {
				Department depament = new Department();
				depament.setDepamentId(resultSet.getInt("department_id"));
				depament.setDepamentName(resultSet.getString("department_name"));
				depament.setCreated(resultSet.getDate("created"));
				depament.setStatus(resultSet.getBoolean("status"));
				depaments.add(depament);
			}
	...
	// Find by Id
		...
		public boolean create(Department department){
			...
			try {
				PreparedStatement preparedStatement = ConnectDB.connection()
						.prepareStatement("insert into department(department_name,created,status) values(?,?,?) ");
			preparedStatement.setString(1, department.getDepamentName());
			preparedStatement.setDate(2,new java.sql.Date( department.getCreated().getTime()));
		...
		public boolean update(Department department){
			...
			try {
				PreparedStatement preparedStatement = ConnectDB.connection()
						.prepareStatement("update department set department_name = ? ,created = ? , status = ? where department_id = ? ");
				preparedStatement.setString(1, department.getDepamentName());
				preparedStatement.setDate(2,new java.sql.Date(department.getCreated().getTime()));
				preparedStatement.setBoolean(3, department.isStatus());
				preparedStatement.setInt(4, department.getDepamentId());
			result = preparedStatement.executeUpdate() > 0;
		...
		// Search by keyword
			public List<Department> searchByKeyword(String keyword) {
				...
				try {
					PreparedStatement preparedStatement = ConnectDB.connection()
							.prepareStatement("select * from department where department_name like ? ");
					preparedStatement.setString(1, keyword + "%");

					ResultSet resultSet = preparedStatement.executeQuery();
					while (resultSet.next()) {
						Department department = new Department();
						department.setDepamentId(resultSet.getInt("department_id"));
						department.setDepamentName(resultSet.getString("department_name"));
